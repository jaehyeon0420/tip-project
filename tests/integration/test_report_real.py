...
@pytest.mark.asyncio
async def test_generate_and_evaluate_report(real_state):
    """실제 vLLM으로 보고서 생성 후, GPT-4o로 평가"""
    
    if not os.getenv("VLLM_API_KEY"):
        pytest.skip("VLLM_API_KEY가 설정되지 않아 테스트를 건너뜁니다.")
        
    if not os.getenv("AZURE_OPENAI_API_KEY"):
        pytest.skip("AZURE_OPENAI_API_KEY가 설정되지 않아 테스트를 건너뜁니다.")

    # 판례 데이터 추가
    real_state["refined_precedents"] = [
        Precedent(
            precedent_no="1",
            case_id="2019허8057",
    ...
    # 1단계: 보고서 생성 (vLLM/Qwen)
    gen_result = await generate_report_node(real_state)

    assert "report_content" in gen_result
    # vLLM 서버가 꺼져있으면 에러 메시지가 반환될 수 있음
    if "보고서 생성 실패" in gen_result["report_content"]:
        pytest.fail(f"보고서 생성 실패: {gen_result['report_content']}")
        
    assert len(gen_result["report_content"]) > 100  # 최소 길이 검증

    # 2단계: 보고서 평가 (GPT-4o)
    real_state["report_content"] = gen_result["report_content"]
    eval_result = await evaluate_report_node(real_state)
    print(f'eval_result : {eval_result}')
    assert "evaluation_score" in eval_result
    assert 0.0 <= eval_result["evaluation_score"] <= 100.0
    assert eval_result["evaluation_decision"] in ["approved", "regenerate"]
    # 피드백은 문자열이어야 함
    assert isinstance(eval_result["evaluation_feedback"], str)

@pytest.mark.asyncio
async def test_generate_report_only(real_state):
    """실제 vLLM만 사용하여 보고서 생성 (생성 단계 검증)"""
    ...
    real_state["refined_precedents"] = [
        Precedent(
            ...
            file_name="상표권침해금지등",
            start_page="0",
            content="상표의 유사 여부는 두 상표를 놓고 그 외관, 호칭, 관념 등을 객관적, 전체적, 이격적으로 관찰하여 거래상 일반 수요자나 거래자가 상표에 대하여 느끼는 직관적 인식을 기준으로 하여 그 상품의 출처에 관하여 오인·혼동을 일으키게 할 우려가 있는지 여부에 따라 판단하여야 한다.",
            is_relevant=True
        )
    ]
    ...
    assert len(gen_result["report_content"]) > 50

@pytest.mark.asyncio
async def test_evaluate_report_only(real_state):
    """실제 GPT만 사용하여 보고서 평가 (평가 단계 검증)"""
    ...
    [보고서 내용]
    두 상표는 '스타벅스'와 '스타벅'으로 호칭이 매우 유사합니다.
    참조 판례(2020다12345)에 따르면 호칭이 유사하여 일반 수요자가 오인할 우려가 있는 경우 침해로 판단합니다.
    따라서 본 건은 상표권 침해 가능성이 높습니다.
    
    [참조 판례]
    - 2020다12345: 호칭 유사성에 대한 판단 기준 제시
    """
    
    real_state["report_content"] = dummy_report
    # 판례 정보도 필요함 (평가 프롬프트에서 base_context를 만드는데 사용됨)
    real_state["refined_precedents"] = [
        Precedent(precedent_no="1", case_id="2020다12345", content="내용", is_relevant=True)
    ]
    ...
    assert "evaluation_score" in eval_result
    # 점수가 숫자로 잘 나오는지 확인
    assert isinstance(eval_result["evaluation_score"], (int, float))
    assert 0 <= eval_result["evaluation_score"] <= 100
